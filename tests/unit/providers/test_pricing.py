...
OPUS = ModelPrice.from_dollars_per_mtok(
    provider="anthropic",
    model="claude-opus-5",
...
HAIKU = ModelPrice.from_dollars_per_mtok(
    provider="anthropic",
...
def _price_list(*prices: ModelPrice, version: str = "2026-08-01") -> PriceList:
    return PriceList(
        ...
        prices=prices if prices else (OPUS, HAIKU),
    )
...
def test_a_quoted_rate_becomes_an_exact_price_per_token() -> None:
    ...
    assert OPUS.per_input_token == Money(nanodollars=15_000)
    assert OPUS.per_cached_input_token == Money(nanodollars=1_500)
    assert OPUS.per_output_token == Money(nanodollars=75_000)
...
def test_usage_is_charged_by_the_kind_of_token_it_reports() -> None:
    ...
    assert OPUS.charge_for(usage) == Money(nanodollars=expected)
...
def test_cached_tokens_are_not_charged_at_the_full_input_rate() -> None:
    ...
    cached = OPUS.charge_for(
        TokenUsage(input_tokens=0, cached_input_tokens=10_000, output_tokens=0)
    )
    uncached = OPUS.charge_for(TokenUsage(input_tokens=10_000, output_tokens=0))
...
def test_the_same_model_from_two_providers_is_two_prices() -> None:
    ...
    prices = _price_list(OPUS, hosted)
    assert prices.price_for("anthropic", "claude-opus-5") is OPUS
    assert prices.price_for("vllm", "claude-opus-5") is hosted


def test_a_model_priced_twice_is_refused() -> None:
    with pytest.raises(ValidationError):
        _price_list(OPUS, OPUS)
...
def test_a_completion_becomes_a_cost_stamped_with_the_version_that_priced_it() -> None:
    ...
    assert cost.amount == OPUS.charge_for(result.usage)
...
def test_the_model_that_served_the_call_is_the_one_that_is_priced() -> None:
    ...
    cost = _price_list().cost_for_completion(
        _result(model="claude-haiku-4-5"),
        provider="anthropic",
        run_id=new_run_id(),
        incurred_at=utc_now(),
    )
    assert cost.model == "claude-haiku-4-5"
    assert cost.amount == HAIKU.charge_for(TokenUsage(input_tokens=1000, output_tokens=500))
